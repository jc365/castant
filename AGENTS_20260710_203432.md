...
│   ├── src/
│   │   ├── domain/
│   │   │   ├── entities/        ← Actor, Director, Casting, Round (con RoundActorEntry), Submission
...
│   │   │   ├── use-cases/       ← CreateActorUseCase, CreateCastingUseCase, SubmitVideoUseCase, SelectActorsForNextRoundUseCase, ManageRoundActorsUseCase
│   │   │   └── dtos/            ← CreateActorInput, CreateCastingInput, SubmitVideoInput, SelectActorsInput, ManageRoundActorsInput
│   │   ├── infrastructure/
...
**Round — roles de actores:**
- `Round.create()` recibe `RoundActorEntry[]` (no `ActorId[]`)
- `RoundActorEntry` es `{ id: ActorId, role: 'actor' | 'preselector' }`
