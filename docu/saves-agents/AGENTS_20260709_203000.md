...
│   │   │   ├── persistence/     ← prismaClient.ts, PrismaActorRepository.ts, PrismaCastingRepository.ts, PrismaDirectorRepository.ts
...
**Para ejecutar tests de integración** se necesita el flag experimental de ESM:
```bash
NODE_OPTIONS="--experimental-vm-modules" npx jest --testPathPatterns="tests/integration"
```

**Gotcha Prisma + Jest ESM:** El cliente generado de Prisma 7 usa `import.meta.url`, lo que requiere `--experimental-vm-modules`. Además, `DATABASE_URL` se configura en `jest.config.js` (no en el test) porque ESM hoists los imports antes de cualquier asignación de variables.
