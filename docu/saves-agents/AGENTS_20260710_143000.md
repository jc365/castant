...
│   ├── src/
│   │   ├── domain/
│   │   │   ├── entities/        ← Actor, Director, Casting, Round (con RoundParticipantEntry), Submission
...
│   │   │   ├── use-cases/       ← CreateActorUseCase, CreateCastingUseCase, SubmitVideoUseCase, SelectActorsForNextRoundUseCase, ManageRoundParticipantsUseCase
│   │   │   └── dtos/            ← CreateActorInput, CreateCastingInput, SubmitVideoInput, SelectActorsInput, ManageRoundParticipantsInput
│   │   ├── infrastructure/
...
- `Round.create()` recibe `RoundParticipantEntry[]` (no `ActorId[]`)
- `RoundParticipantEntry` es `{ id: ActorId, role: 'actor' | 'preselector' }`
...
- `detectAndAddActors(inputs)` — crea o recupera actores por email, retorna `Set<string>` de IDs
