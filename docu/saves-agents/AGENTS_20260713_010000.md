...
- `Participant` en Prisma vincula Users a Castings con un `role`

**Round — roles de participantes:**
- `Round.create(number, castingId, participants, id?)` — ID es opcional
...
- `POST /api/v1/castings` → crea casting (body: `{ title, description, directorEmail, directorName }`)
