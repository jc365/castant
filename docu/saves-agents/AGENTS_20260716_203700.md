...
- **`LoginForm`** (`src/components/LoginForm.tsx`): Formulario con email/password. Llama a `POST /api/v1/auth/login`, almacena `token` y `userId` en localStorage. Al hacer login exitoso, recarga el estado del Layout para mostrar el contenido.
