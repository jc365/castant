...
├── frontend/
│   ├── src/
│   │   ├── api/                 ← client.ts (Axios con interceptores JWT/demo/debug)
│   │   ├── components/          ← Layout.tsx, LoginForm.tsx
│   │   ├── context/             ← UserContext.tsx (usuario + participaciones + polling)
│   │   ├── pages/               ← Dashboard.tsx, Castings.tsx, CreateCasting.tsx, CastingDetail.tsx, RoundDetail.tsx
│   │   ├── App.tsx              ← Rutas + UserProvider
...
- `LoginUseCase` soporta login demo via `xUserId` opcional en `LoginInput` — si se provee, valida que `DEMO_MODE=true` antes de permitir el login; si no, lanza error `'Demo mode is disabled'`
...
- LoginUseCase valida `DEMO_MODE` cuando se provee `xUserId` en el body — si `DEMO_MODE=false`, lanza error `'Demo mode is disabled'`
- LoginUseCase valida `xUserId` contra mapa `DEMO_USERS` — roles válidos: `director`, `actor`, `preselector` — si no existe, lanza error `'Invalid demo role'`
...
- **Modo Demo:** Toggle en la parte inferior de la sidebar. Solo visible si `VITE_DEMO_MODE=true` en `.env` del frontend. Al activarse, inyecta header `X-User-Id` en todas las peticiones API.
- **Selector de Roles:** Desplegable con opciones: director, actor, preselector. Solo visible cuando el modo Demo está activo. El valor se guarda en `localStorage` (`demo-role`) y se inyecta como `X-User-Id` (ej. `user-director-1`).
- **API Client:** Axios interceptor en `src/api/client.ts` que añade `X-User-Id` de `localStorage` a todas las peticiones.
...
- **Cuando no hay autenticación** (`!token && !demoMode`): `LoginForm` se renderiza en el área principal del Dashboard (donde iría `<Outlet />`).
- **Cuando hay token o modo Demo activado**: Se muestra el contenido normal del dashboard (rutas hijas).
...
- **`handleLogout`** en Layout: Limpia `token`, `userId` y `x-user-id` de localStorage. No redirige a ninguna ruta; solo actualiza el estado para que aparezca el `LoginForm`.
- **Ruta `/login`**: Redirige a `/dashboard` (el LoginForm se muestra automáticamente si no hay auth).
- **Modo Demo**: Si se activa el toggle de Demo, el usuario accede al contenido sin necesidad de token. El header `X-User-Id` se inyecta automáticamente en las peticiones.
...
- **`UserContext`** (`src/context/UserContext.tsx`): Proveedor de contexto que gestiona el usuario autenticado y sus participaciones.
  ...
  - `isAuthenticated`: estado reactivo derivado de `localStorage.token` / `localStorage['demo-mode']`.
...
- **`RoundDetail`** (`src/pages/RoundDetail.tsx`): Detalle de una ronda con participants, submissions, y botones según el rol (ej. "Submit Video" para actors, "Review" para directors/preselectors).

### Frontend Testing

**Unit/Component Tests (Vitest + React Testing Library):**
- Configuración: `frontend/vitest.config.ts` con `jsdom` environment
- Setup: `frontend/src/test/setup.ts` (cleanup automático + jest-dom)
