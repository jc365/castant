...
- **`index.css`**: Define `:root` (light palette con colores MD3) y `.dark` (dark palette existente)
...
- **Layout.tsx**: Botón de toggle (sun/moon icon) en la sección inferior de la sidebar
...
- **Dark/Light Mode:** Theme toggle en la sidebar inferior. Usa CSS custom properties en `:root` (light) y `.dark` (dark). `ThemeContext` gestiona estado con persistencia en `localStorage` y detección de preferencia del sistema.
